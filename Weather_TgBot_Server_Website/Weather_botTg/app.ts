...
const mqttClient = mqtt.connect(process.env.WEATHER_MQTT_BROKER_URL_PORT!, {
    username: process.env.WEATHER_MQTT_USERNAME,
    password: process.env.WEATHER_MQTT_PASSWORD
});
...
function handleReceivedInfo(info: CommonInfo) {
	...
	if (info.selected_device == "no_selected") {
        bot.telegram.sendMessage(
            userId,
            "Не вибрано пристрою"
        );
